import { MultiTimeframeManager, TimeframeConverter } from '../src/engine/multiTimeframe.js';
...
describe('MultiTimeframeManager', () => {
  ...
  describe('Timeframe Registration', () => {
    test('should register timeframe successfully', () => {
      manager.addTimeframe('1h', mockStrategy);
      
      expect(manager.timeframes.has('1h')).toBe(true);
      expect(manager.timeframes.get('1h').strategies).toContain(mockStrategy);
    });
    
    test('should register multiple strategies for same timeframe', () => {
      const strategy2 = { name: 'Strategy2', onCandle: jest.fn() };
      
      manager.addTimeframe('1h', mockStrategy);
      manager.addTimeframe('1h', strategy2);
      
      const timeframe = manager.timeframes.get('1h');
      expect(timeframe.strategies).toHaveLength(2);
      expect(timeframe.strategies).toContain(mockStrategy);
      expect(timeframe.strategies).toContain(strategy2);
    });
    
    test('should not register same strategy twice for same timeframe', () => {
      manager.addTimeframe('1h', mockStrategy);
      manager.addTimeframe('1h', mockStrategy);
      
      const timeframe = manager.timeframes.get('1h');
      expect(timeframe.strategies).toHaveLength(1);
    });
  });
  
  describe('Timeframe Removal', () => {
    test('should remove strategy from timeframe', () => {
      manager.addTimeframe('1h', mockStrategy);
      manager.removeTimeframe('1h', mockStrategy);
      
      const timeframe = manager.timeframes.get('1h');
      expect(timeframe.strategies).not.toContain(mockStrategy);
    });
    
    test('should remove timeframe when no strategies left', () => {
      manager.addTimeframe('1h', mockStrategy);
      manager.removeTimeframe('1h', mockStrategy);
      
      expect(manager.timeframes.has('1h')).toBe(false);
    });
  });
  
  describe('Candle Processing', () => {
    test('should process candle for correct timeframe', () => {
      manager.addTimeframe('1m', mockStrategy);
      manager.addTimeframe('5m', mockStrategy);
  ...
  describe('Timeframe Validation', () => {
    test('should validate supported timeframes', () => {
      expect(() => manager.addTimeframe('1m', mockStrategy)).not.toThrow();
      expect(() => manager.addTimeframe('5m', mockStrategy)).not.toThrow();
      expect(() => manager.addTimeframe('15m', mockStrategy)).not.toThrow();
      expect(() => manager.addTimeframe('1h', mockStrategy)).not.toThrow();
      expect(() => manager.addTimeframe('4h', mockStrategy)).not.toThrow();
      expect(() => manager.addTimeframe('1d', mockStrategy)).not.toThrow();
    });
    
    test('should reject invalid timeframes', () => {
      expect(() => manager.addTimeframe('2m', mockStrategy)).toThrow('Unsupported timeframe: 2m');
      expect(() => manager.addTimeframe('invalid', mockStrategy)).toThrow('Unsupported timeframe: invalid');
    });
  });
...
describe('TimeframeConverter', () => {
  ...
  describe('Timeframe Relationships', () => {
    ...
    test('should throw error for invalid conversions', () => {
      ...
      expect(() => converter.getConversionRatio('1m', '3m')).toThrow('Target timeframe is not evenly divisible by source timeframe');
    });
  });
